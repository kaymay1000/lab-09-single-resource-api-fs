...
describe('server module', function() {
  ...
  describe('GET method', function() {
    ...
    before(done => {
      ...
      .end((err, res) => {
        getResource = JSON.parse(res.body.toString());
        console.log(res);
        done();
      });
    });
  ...
  // describe('POST method', function() { //don't need before block with mock data (or after block) because there doesn't need to be any pre-existing data to test this method!
  //   describe('a properly formatted request', function() {
  ...
  //         .send({name: 'Kaylee', date: 'April 19, 2017'})
  //         .end((err, res) => {
  //           if(err) console.error(err);
  //           expect(res.status).to.equal(400);
  //           done();
